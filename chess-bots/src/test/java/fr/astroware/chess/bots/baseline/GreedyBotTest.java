...
class GreedyBotTest {
    ...
    @Test
    void capturesMostValuableAvailablePiece() {
        Move queenCapture = Move.of("b3", "d5");
        Move pawnCapture = Move.of("h1", "h7");
        ...
        BotContext context = new TestContext(
            ...
            List.of(queenCapture, pawnCapture)
        );

        BotDecision decision = new GreedyBot().decide(context);

        assertEquals(queenCapture, decision.move());
        assertEquals(
